...
public class YishengGuahaoController {
    private static final Logger logger = LoggerFactory.getLogger(YishengGuahaoController.class);

    private static final String TABLE_NAME = "yishengGuahao";

    @Autowired
    private YishengGuahaoService yishengGuahaoService;
    ...
    */
    ...
    public R page(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        PageUtils page = yishengGuahaoService.queryPage(params);

        //字典表数据转换
        List<YishengGuahaoView> list =(List<YishengGuahaoView>)page.getList();
        for(YishengGuahaoView c:list){
            //修改对应字典表字段
    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        YishengGuahaoEntity yishengGuahao = yishengGuahaoService.selectById(id);
        if(yishengGuahao !=null){
            //entity转view
            YishengGuahaoView view = new YishengGuahaoView();
            BeanUtils.copyProperties( yishengGuahao , view );//把实体数据重构到view中
            //级联表 病人
            //级联表
            BingrenEntity bingren = bingrenService.selectById(yishengGuahao.getBingrenId());
            if(bingren != null){
            ...
            //级联表 医生
            //级联表
            YishengEntity yisheng = yishengService.selectById(yishengGuahao.getYishengId());
            if(yisheng != null){
    ...
    */
    @RequestMapping("/save")
    public R save(@RequestBody YishengGuahaoEntity yishengGuahao, HttpServletRequest request){
        ...
        Wrapper<YishengGuahaoEntity> queryWrapper = new EntityWrapper<YishengGuahaoEntity>()
        ...
        YishengGuahaoEntity yishengGuahaoEntity = yishengGuahaoService.selectOne(queryWrapper);
        if(yishengGuahaoEntity==null){
            yishengGuahao.setInsertTime(new Date());
            yishengGuahao.setCreateTime(new Date());
            yishengGuahaoService.insert(yishengGuahao);
            return R.ok();
        }else {
    ...
    */
    ...
    public R update(@RequestBody YishengGuahaoEntity yishengGuahao, HttpServletRequest request) throws NoSuchFieldException, ClassNotFoundException, IllegalAccessException, InstantiationException {
        logger.debug("update方法:,,Controller:{},,yishengGuahao:{}",this.getClass().getName(),yishengGuahao.toString());
        YishengGuahaoEntity oldYishengGuahaoEntity = yishengGuahaoService.selectById(yishengGuahao.getId());//查询原先数据
...
//            yishengGuahao.setYishengId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
            ...
            yishengGuahaoService.updateById(yishengGuahao);//根据id更新
            return R.ok();
    }
    ...
    */
    ...
    public R delete(@RequestBody Integer[] ids, HttpServletRequest request){
        ...
        List<YishengGuahaoEntity> oldYishengGuahaoList =yishengGuahaoService.selectBatchIds(Arrays.asList(ids));//要删除的数据
        yishengGuahaoService.deleteBatchIds(Arrays.asList(ids));
    ...
    public R save( String fileName, HttpServletRequest request){
        ...
        try {
            List<YishengGuahaoEntity> yishengGuahaoList = new ArrayList<>();//上传的东西
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            YishengGuahaoEntity yishengGuahaoEntity = new YishengGuahaoEntity();
...
//                            yishengGuahaoEntity.setCreateTime(date);//时间
                        ...
                        //查询是否重复
                         //挂号编号
                        List<YishengGuahaoEntity> yishengGuahaoEntities_yishengGuahaoUuidNumber = yishengGuahaoService.selectList(new EntityWrapper<YishengGuahaoEntity>().in("yisheng_guahao_uuid_number", seachFields.get("yishengGuahaoUuidNumber")));
                        if(yishengGuahaoEntities_yishengGuahaoUuidNumber.size() >0 ){
                            ArrayList<String> repeatFields = new ArrayList<>();
                            for(YishengGuahaoEntity s:yishengGuahaoEntities_yishengGuahaoUuidNumber){
                                repeatFields.add(s.getYishengGuahaoUuidNumber());
                            }
                            return R.error(511,"数据库的该表中的 [挂号编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
                        yishengGuahaoService.insertBatch(yishengGuahaoList);
                        return R.ok();
                    }
                }
            }
    ...
    */
    @RequestMapping("/add")
    public R add(@RequestBody YishengGuahaoEntity yishengGuahao, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,yishengGuahao:{}",this.getClass().getName(),yishengGuahao.toString());

        List<YishengGuahaoEntity> yishengGuahaoEntityList = yishengGuahaoService.selectList(new EntityWrapper<YishengGuahaoEntity>()
                .eq("yisheng_id", yishengGuahao.getYishengId())
                .eq("yisheng_guahao_time", new SimpleDateFormat("yyyy-MM-dd").format(yishengGuahao.getYishengGuahaoTime()))
                .notIn("yisheng_guahao_types", new Integer[]{102})
        );

        YishengEntity yishengEntity = yishengService.selectById(yishengGuahao.getYishengId());
        ...
        BingrenEntity bingrenEntity = bingrenService.selectById(yishengGuahao.getBingrenId());
    ...
    */
    ...
    public R refund(Integer id, HttpServletRequest request){
        ...
        String role = String.valueOf(request.getSession().getAttribute("role"));
            ...
            YishengGuahaoEntity yishengGuahao = yishengGuahaoService.selectById(id);//当前表service
    ...
    public R deliver(Integer id  , HttpServletRequest request){
        ...
        YishengGuahaoEntity  yishengGuahaoEntity = yishengGuahaoService.selectById(id);
        yishengGuahaoEntity.setYishengGuahaoTypes(103);//设置订单状态为已使用
        yishengGuahaoService.updateById( yishengGuahaoEntity);
